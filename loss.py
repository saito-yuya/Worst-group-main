...
class LossComputer:
    def __init__(self, criterion, is_robust, dataset, loss_type = 'ce',alpha=None, vs_alpha=0.0, gamma=0.1, adj=None, min_var_weight=0, step_size=0.01, normalize_loss=False, btl=False):
    ...
    def loss(self, yhat, y, group_idx=None, is_training=False):
        ...
        #print(yhat,y,group_idx)
        #print('paused inside loss()')
        #time.sleep(10**3)
        if self.loss_type == 'cdt':
            per_sample_losses = self.criterion(yhat, y, group_idx)
        elif self.loss_type == 'vs': 
            per_sample_losses = self.criterion(yhat, y, group_idx)
        elif self.loss_type == 'la':
            per_sample_losses = self.criterion(yhat, y, group_idx)
        elif self.loss_type == 'la_tau':
            per_sample_losses = self.criterion(yhat, y, group_idx)
        elif self.loss_type == 'vs_tau':
            per_sample_losses = self.criterion(yhat, y, group_idx)
    ...
    def compute_group_avg(self, losses, group_idx):
       ...
       # print('inside compute_group_avg')
       # print('group map')
       # print(group_map)
       # print('losses')
       # print(losses.view(-1))
...
class MiniLossComputer:
    ...
    def loss(self, yhat, y, group_idx=None):
        ...
        # per_sample_losses = self.criterion(yhat, y)
        # group_loss, group_count = self.compute_group_avg(per_sample_losses, group_idx)
        group_acc, group_count = self.compute_group_avg((yhat==y).float(), group_idx)
        return group_acc, group_count
